...
def my_model(spark,
    ...
    #
    filteredSDF = time_inputSDF.select(f.substring("date",12,2).alias("hour"),f.col("congestion"),f.col("my_time")).where((f.col("latitude") >= south) & \
    ...
    filteredSDF.createOrReplaceTempView("filteredView")
    
    hourFilteredSDF = spark.sql("""select hour,congestion,congestion as cong,my_time from filteredView where hour in {} """.format(( *hours_list,)))

                               
    watermarkSDF = hourFilteredSDF.withWatermark("my_time", "0 seconds") \
                     .groupBy(pyspark.sql.functions.window("my_time", my_frequency, my_frequency),
                              pyspark.sql.functions.col("hour")
                             )\
                     .agg({"congestion":"sum","*":"count","cong":"avg"})\
    ...
    # get the final SDF
    solutionSDF =   watermarkSDF.select(f.col("hour"),\
                             (f.round(f.col("percentage")*100,2)).alias("percentage"),\
                              f.col("numMeasurements"),\
                              f.col("congestionMeasurements"))
