...
def my_model(spark,
    ...
    # Filter row based on time windows and atStop
    busStoppedSDF = time_inputSDF.select(f.col("*"))\
    .where((f.col("atStop") == 1) & (f.to_timestamp(time_inputSDF["date"])>=star_time) & (f.to_timestamp(time_inputSDF["date"])<=cutoff_time))
    
    # select  busstop id and vehicleid.. other columns not needed
    filteredSDF = busStoppedSDF.select(f.col("closerStopID"),f.col("vehicleID"),f.col("my_time"))
    ...
    # group the row based on busstopid   and form sorted array of vechileid for the group                 
