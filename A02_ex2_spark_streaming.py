...
def filter_value(row_list):
   ...
   for item in row_list:
      ...
      if(previous_item == None or previous_item[0]!=current_item[0]):
         return_value.append(current_item)
               
   return return_value
    

def flat_map_fun(row_list):
    return_list = []
    key = row_list[0]
    for item in row_list[1]:
       return_list.append((key,item[0],item[1],item[2]))
...
def my_model(ssc,
    ...
    allWordsDStream = windowDStream.map(process_line)
    
    filterDStream = allWordsDStream.filter(lambda row: \
                                          row[7] == vehicle_id and \
                                          row[9] != 0 and \
                                          row[0][:10] == day_picked)

    uniqueRowDStream = filterDStream.map(lambda row: (row[1], row[8], datetime.datetime.strptime(row[0], "%Y-%m-%d %H:%M:%S"), row[6])) \
                                .transform(lambda rdd: rdd.sortBy(lambda row: (row[0],row[2]))) \
                                .map(lambda row: ((row[0]),(row[1],row[2],row[3]))) \
                                .groupByKey()\
                                .mapValues(lambda row: filter_value(list(row))) \
                                .flatMap(flat_map_fun)
    lineidsortedDStream = uniqueRowDStream.transform(lambda rdd: rdd.sortBy(lambda row: row[2]))
    
    solutionDStream = lineidsortedDStream.map(lambda row:(row[0],row[1],row[2].strftime('%H:%M:%S'),1 if abs(row[3])<=abs(delay_limit) else 0))
