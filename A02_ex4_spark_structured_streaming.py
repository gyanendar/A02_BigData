...
def my_model(spark,
    ...
    rawSDF = time_inputSDF.withColumn("date",f.to_timestamp(time_inputSDF["date"]))
    
    dateFilteredSDF = rawSDF.select(f.col("date"),f.col("vehicleID"),f.col("closerStopID"),f.col("my_time"))\
                            .where(f.to_date(rawSDF["date"]) == f.to_date(f.lit(day_picked)))
                            
    solutionSDF =   dateFilteredSDF.groupBy(pyspark.sql.functions.col("vehicleID")) \
                                  .agg(f.approx_count_distinct("closerStopID").alias("numStations"))\
                                  .orderBy(pyspark.sql.functions.col("vehicleID").desc())                        
    # ---------------------------------------
